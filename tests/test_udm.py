...
class TestUDMClient(unittest.TestCase):
    ...
    def test003_user_crud(self):
        ...
        for key, val in new_props.items():
            if key == 'password':
                continue
            self.assertEqual(udm_json['_embedded']['udm:object'][0]['properties'][key], val)
        # modify user
    ...
    def test004_group_crud(self):
        ...
        udm_res = self.udm_client.add(
            UDMModel.GROUP,
            {
                'name': new_groupname,
                'description': 'just an automated test',
                'users': [f'uid=Administrator,cn=users,{self.udm_client.base_position}'],
            },
    ...
    def test005_single_valued_props(self):
        sv_prop_names = self.udm_client.single_valued_props(UDMModel.USER)
        sv_prop_names = self.udm_client.single_valued_props(UDMModel.GROUP)
    ...
    def test006_ou_created(self):
        try:
            ...
            raise AssertionError('User and group OU should be created if they does not exist')
